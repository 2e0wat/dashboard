...
for ($i=0;$i<$Reflector->NodeCount();$i++) {
   ...
   if ($ShowThisStation) {
      ...
      echo '
      <tr height="30" bgcolor="'.$odd.'" onMouseOver="this.bgColor=\''.$seclt.'\';" onMouseOut="this.bgColor=\''.$odd.'\';" style="cursor:pointer" onclick=window.open("http://www.aprs.fi/'.$Reflector->Nodes[$i]->GetCallSign();
      if ($Reflector->Nodes[$i]->GetSuffix() != "") echo '-'.$Reflector->Nodes[$i]->GetSuffix(); echo '")>
      ...
      if (file_exists("./img/flags/".$Flag.".png")) {
         echo '<img src="./img/flags/'.$Flag.'.png" height="15" '.$Name.' />';
      }
      echo '</td>
	...
	'.$Reflector->Nodes[$i]->GetCallSign();
	 if ($Reflector->Nodes[$i]->GetSuffix() != "") { echo '-'.$Reflector->Nodes[$i]->GetSuffix(); } echo '
      ...
      <td width="130" align="center">'.date("d/m/y H:i", $Reflector->Nodes[$i]->GetLastHeardTime()).'</td>
      <td width="130" align="center">'.FormatSeconds(time()-$Reflector->Nodes[$i]->GetConnectTime()).'</td>
      <td width="100" align="center">'.$Reflector->Nodes[$i]->GetProtocol().'</td>
      <td width="60" align="center">'.$Reflector->Nodes[$i]->GetLinkedModule().'</td>';
      if ($PageOptions['RepeatersPage']['IPModus'] != 'HideIP') {
         echo '
